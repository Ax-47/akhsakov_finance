...
fn TabButton(label: String, mut active_tab: Signal<String>) -> Element {
    ...
    rsx! {
        button {
            class: if is_active() {
                            "px-5 py-3 text-sm font-medium cursor-pointer border-b-2 border-ctp-blue text-white"
                        } else {
                            "px-5 py-3 text-sm font-medium cursor-pointer border-b-2 border-transparent text-subtext0 hover:text-white"
                        },
