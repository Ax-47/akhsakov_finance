...
pub fn download_base64(filename: &str, mime: &str, data: &str) {
    let script = format!(
        "const bin = atob({});
...
pub fn print_report() {
    document::eval(
        "const flavours = ['mocha', 'macchiato', 'frappe'];
         const dark = [];
         flavours.forEach(f => document.querySelectorAll('.' + f).forEach(e => {
             dark.push([e, f]);
             e.classList.replace(f, 'latte');
         }));
         const restore = () => {
             dark.forEach(([e, f]) => e.classList.replace('latte', f));
